...
const items = [
  {
    icon: Lock,
    title: "Passcode Lock",
    text: "PIN + biometric protection on every launch.",
  },
  {
    icon: CloudUpload,
    title: "Secure Backup",
    text: "Encrypted backups you fully control.",
  },
  {
    icon: RefreshCw,
    title: "Restore System",
    text: "One-tap restore on any new device.",
  },
  {
    icon: Smartphone,
    title: "Private Storage",
    text: "Data stays on your device by default.",
  },
];
...
const SecurityFeature = ({
  ...
  <div className="rounded-2xl bg-background/5 border border-background/10 backdrop-blur p-5 hover:bg-background/10 transition-colors">
    <div className="h-11 w-11 rounded-xl bg-primary/20 flex items-center justify-center mb-3">
      <Icon className="h-5 w-5 text-primary" />
    </div>
    <h3 className="text-base font-bold text-background">{title}</h3>
        ...
        <div className="relative rounded-[2rem] overflow-hidden bg-foreground text-background p-8 sm:p-12 lg:p-16 shadow-lg-soft">
          ...
          <div className="relative max-w-3xl">
            ...
            <p className="mt-4 text-lg text-background/70">
              Privacy isn't a feature — it's the foundation. MoneyFlow Manager
              keeps your financial life private, encrypted, and entirely in your
              hands.
            </p>
          </div>
          ...
          <div className="relative mt-10 grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {items.map((item) => (
              <SecurityFeature key={item.title} {...item} />
            ))}
          </div>
        </div>
      </div>
